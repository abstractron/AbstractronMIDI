    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_506 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_506.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_506 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_506.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_133 = pow((abs((frac((o.uv0+node_5758.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                float3 lightColor = _LightColor0.rgb;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 _Normals_var = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(i.uv0, _Normals)));
                float4 node_5758 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_5758.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float3 normalLocal = normalize(lerp(_Normals_var.rgb,float3(0,0,1),node_133));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv2 = v.texcoord2;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_3540 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_3540.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                o.pos = UnityObjectToClipPos(v.vertex );
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_133 = pow((abs((frac((o.uv0+node_5581.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 node_5581 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_5581.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
